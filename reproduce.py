...
# Add slopes N and (N+1)/2
def plot_slope(N, x_range, y_intercept):
    x = np.array(x_range)
    y_N = 10 ** ((-x * N) / 10 + y_intercept)
    y_half_N = 10 ** (-x * (N + 1) / 2 / 10)

    plt.plot(x, y_N, linestyle='--', color="m", label=f'Slope = {N}')
...
def outage_prob_plot(fig_num):
    if fig_num == 1:
        ...
        plot_slope(2, np.linspace(37.5, 45, 1000), 2)
        plot_slope(4, np.linspace(17.5, 45, 1000), 1.5)
        plot_slope(8, np.linspace(5.5, 45, 100), -2)
        ...
        fig_name = "The outage probability versus the SNR_1e7"

        opt_P_out_values_2 = np.load(f"./Curves/SC_opt_outage_prob_2.npy")
        opt_P_out_values_4 = np.load(f"./Curves/SC_opt_outage_prob_4.npy")
        opt_P_out_values_8 = np.load(f"./Curves/SC_opt_outage_prob_8.npy")

        plt.figure(figsize=(8, 8))
        plt.title('Outage Probability vs. Transmit SNR')

        plt.plot(10 * np.log10(opt_P_out_values_2[0]), opt_P_out_values_2[1], 'm>-',
                 label=f'Optimal continuous phase-shift')
        plt.plot(10 * np.log10(opt_P_out_values_4[0]), opt_P_out_values_4[1], 'mo-')
        plt.plot(10 * np.log10(opt_P_out_values_8[0]), opt_P_out_values_8[1], 'm^-')
