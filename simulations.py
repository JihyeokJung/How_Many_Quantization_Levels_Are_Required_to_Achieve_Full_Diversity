...
def Diversity_gain_similation(Omega_S, Omega_I):
    N_values = np.linspace(2, 30, 29)

    diversity_gain = np.array([get_Div_gain(int(n), Omega_S, Omega_I) for n in N_values])

    np.save(f"./Curves/diversity_gain", np.array([N_values, diversity_gain]))
